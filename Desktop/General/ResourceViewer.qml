...
ESborder {
   ...
   Column {
    ...
    MarkDown {
        id:mediaView
        visible: if(media.split("![")[1] !== undefined && media.split("![VID]")[1] === undefined) {true} else {false}
        enabled: visible
        width:parent.width
        thedata: media
    ...
    Item {
        ...
        visible: if(mediaView.visible === true || video.visible === true){false} else {true}
        onVisibleChanged: { webview.url = media.split("](")[1].split(")")[0]
