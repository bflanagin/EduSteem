function saveSchool(userid, type, name, email, phone, country, state, about, schoolcode) {

    /*saves school */
    db.transaction(function (tx) {
        var data = [userid, type, name.replace(
                        / /g,
                        "_"), email, phone, country, state, about, schoolcode]
        var dtable = "INSERT INTO Schools VALUES(?,?,?,?,?,?,?,?,?,?)"
        var update = "UPDATE Schools SET type=" + type + ", name='"
                + name.replace(/ /, "_") + "',  email='" + email + "', phone='"
                + phone + "', country='" + country + "', state='"
                + state + "', about='" + about + "' WHERE id='" + userid + "'"
...
function findSchools() {/* Checks online list for schools in eduSteem for naming conflicts. */
}
